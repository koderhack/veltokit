...
/// Pong + Breakout: żyro, klocki wielopoziomowe, punkty, dźwięk.
final class PongGame: Game {
  ...
  func render(context: GameContext) {
    ...
    context.text("PONG", x: 4, y: 3, color: .white)
    context.text("SC \(score)", x: 42, y: 3, color: .yellow)
    context.text("L \(lives)", x: 118, y: 3, color: .cyan)
