...
/// Paletka: BLE zapisuje rawX; updateFrame @ 60 FPS (bez skanowania bufora co klatkę).
@MainActor
public final class MotionSDK: ObservableObject {
  ...
  /// Ostatni raw z notify — konsumowany raz na klatkę @ 60 FPS.
  private var pendingPaddleRaw: Double?
  ...
  public func flushIngress() {
    if engine.config.mode == .paddle {
      if let raw = pendingPaddleRaw {
        engine.setRawX(raw)
        pendingPaddleRaw = nil
      }
      return
    }
  ...
  public func ingestTrikiFrame(
    ...
    if engine.config.mode == .paddle {
      pendingPaddleRaw = mapped.x * BLEGyroParser.gyroDivisor
    } else {
